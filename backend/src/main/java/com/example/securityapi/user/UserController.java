...
public class UserController {
    ...
    public List<UserResponse> studentProfileUsers() {
        Set<Long> studentUserIds = memberships
        ...
        return users.findAllById(studentUserIds).stream()
                .map(AuthService::toResponse)
                .sorted((left, right) ->
    ...
    @GetMapping("/teacher-profile-users")
    public List<UserResponse> teacherProfileUsers() {
        Set<Long> teacherUserIds = memberships
                .findByMembershipTypeAndMembershipStatusIgnoreCaseOrderByUserId(
        ...
        return users.findAllById(teacherUserIds).stream()
                .map(AuthService::toResponse)
                .sorted((left, right) ->
                        left.email().compareToIgnoreCase(right.email()))
                .toList();
