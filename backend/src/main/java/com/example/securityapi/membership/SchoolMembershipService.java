...
public class SchoolMembershipService {
    ...
    public List<MembershipResponse> findAll() {
        return memberships.findAll()
                .stream()
                .map(this::toResponse)
                .sorted(
                        Comparator.comparing(
                                        MembershipResponse::schoolName,
                                        String.CASE_INSENSITIVE_ORDER
                                )
                                .thenComparing(
                                        MembershipResponse::userEmail,
                                        String.CASE_INSENSITIVE_ORDER
                                )
                )
                .toList();
    ...
    private void validate(
        ...
        if (membershipExists(request, excludedId)) {
            throw new ResponseStatusException(
                    HttpStatus.CONFLICT,
    ...
    private boolean membershipExists(
        ...
        /*
         * startDate is nullable. Use separate queries so PostgreSQL
         * does not need to infer the SQL type of a null parameter.
         */
        if (request.startDate() == null) {
            if (excludedId == null) {
                return memberships
                        .existsByUserIdAndSchoolIdAndMembershipTypeAndStartDateIsNull(
                                request.userId(),
                                request.schoolId(),
                                request.membershipType()
                        );
            ...
            return memberships
                    .existsByUserIdAndSchoolIdAndMembershipTypeAndStartDateIsNullAndIdNot(
                            request.userId(),
                            request.schoolId(),
                            request.membershipType(),
                            excludedId
        ...
        if (excludedId == null) {
            return memberships
                    .existsByUserIdAndSchoolIdAndMembershipTypeAndStartDate(
                            request.userId(),
                            request.schoolId(),
                            request.membershipType(),
                            request.startDate()
        ...
        return memberships
                .existsByUserIdAndSchoolIdAndMembershipTypeAndStartDateAndIdNot(
                        request.userId(),
                        request.schoolId(),
                        request.membershipType(),
                        request.startDate(),
    ...
    private SchoolMembership requireMembership(
            Long id
    ) {
    ...
    private MembershipResponse toResponse(
        ...
        School school =
                requireSchool(membership.getSchoolId());
        ...
        return new MembershipResponse(
                ...
                membership.getExternalPersonId(),
